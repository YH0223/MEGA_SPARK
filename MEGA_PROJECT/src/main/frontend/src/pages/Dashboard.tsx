...
const Dashboard = () => {
  ...
  const fetchTaskProgress = async (projectId: number) => {
    try {
      const response = await api.get(`/task/progress/${projectId}`, { withCredentials: true });
  ...
  const fetchProjectStatus = async () => {
    try {
      const response = await api.get("/projects/status", { withCredentials: true });
  ...
  useEffect(() => {
    fetchUserProfile();
    fetchProjects();
    api.get("/projects/status", { withCredentials: true })
        .then(response => {
